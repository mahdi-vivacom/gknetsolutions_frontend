...
export const EditLeadPage = () => {
  ...
  const { data: lead, isLoading: isLoadingLead } = useQuery<any>({
    queryKey: ["lead", id],
    queryFn: () => fetchAPI<any>(`/leads/${id}`),
    enabled: !!id,
  });
  ...
  useEffect(() => {
    if (lead) {
      setFormData({
        ...
        expectedUsers: lead.expectedUsers || "",
        estimatedBudget: lead.estimatedBudget?.toString() || "",
        currency: lead.currency || "BDT",
        budgetRange: lead.budgetRange || "",
        expectedStartDate: lead.expectedStartDate ? lead.expectedStartDate.split('T')[0] : "",
        expectedDeliveryTimeline: lead.expectedDeliveryTimeline || "",
        notes: lead.notes || "",
      });
    }
  }, [lead]);
    ...
    <div className="space-y-6 animate-fade-in font-sans pb-12">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-foreground">Edit Lead</h2>
        ...
        <Button 
          ...
          className="bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white rounded-xl shadow-md h-10 px-6 font-semibold"
        >
